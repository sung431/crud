...
$(document).ready(function(){
	$("#BtnMod").click(function(){
		var insertData = $("#form").serialize();
 		$.ajax({
		url : "/crud/member/memberUpdate.do"
		, data : insertData
		, type : "POST"
		, dataType : "json"
		, error : function(){
		...
		, success : function(response) {
			if(response == 1){
				alert("수정완료");
				location.href="/crud/member/memberList.do";
			}else{				
				alert("수정실패 : " + reponse);
			}
		},
	...
	$("#BtnDel").click(function(){
		if(confirm("삭제하시겠습니까?") == true){
			var insertData = $("#form").serialize();
	 		$.ajax({
			url : "/crud/member/memberDelete.do"
			, data : insertData
			, type : "POST"
			, dataType : "json"
			, error : function(){
	...
	<div class="content">
		...
		<div class="col-2">
			...
			<div class="write_form kor active">
				<label for="">부서</label><select id="deptNo" name="deptNo">
					<c:choose>
						<c:when test="${fn:length(list) > 0}">
			                <c:forEach items="${list }" var="row">
			                		<c:if test="${row.deptNo == dto.deptNo }">
